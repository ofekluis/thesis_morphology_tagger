...
def extract_data(path,lang):
    """
    loops over all files with .tag extension in path and extract from their
    ...
    if lang=="nl":
        ...
        skip="nl"
    with open(os.path.join(path, "x_all_"+lang), "w") as x_all:
        with open(os.path.join(path, "y_all_"+lang), "w") as y_all:
...
def divide_data(path,lang):
    ...
    with open(os.path.join(path, "x_all_" + lang), "r") as x_all:
        with open(os.path.join(path, "y_all_" + lang), "r") as y_all:
            with open(os.path.join(path, "x_trn_" + lang), "w") as x_trn:
                with open(os.path.join(path, "y_trn_" + lang), "w") as y_trn:
                    with open(os.path.join(path, "x_vld_" + lang), "w") as x_vld:
                        with open(os.path.join(path, "y_vld_" + lang), "w") as y_vld:
                            with open(os.path.join(path, "x_tst_" + lang), "w") as x_tst:
                                with open(os.path.join(path, "y_tst_" + lang), "w") as y_tst:
                                    fileDic={
                                        "trn":(x_trn, y_trn),
                                        "vld":(x_vld, y_vld),
                                        "tst":(x_tst, y_tst)
...
def main():
    ...
    parser.add_argument("--path", help="Choose path (recursive)", default="./")
    args = parser.parse_args()

    extract_data(args.path,args.lang)
    divide_data(args.path,args.lang)
